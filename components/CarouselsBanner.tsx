...
const CarouselsBanner = ({ movies }: Props) => {
  ...
  return (
    ...
    >
        ...
        {movies.map((movie) => (
          <div key={movie.id} className="flex-full min-w-0 relative">
            ...
            <div className="hidden md:inline absolute mt-0 top-0 pt-40 xl:pt-52 left-0 lg:mt-40 bg-transparent z-20 h-full w-full bg-gradient-to-r from-gray-900/90 via-transparent to-transparent p-10 space-y-5 text-white">
              ...
              <p className="max-w-xl line-clamp-3 text-green-500 font-semibold">
                Rating {movie.vote_average}%{" "}
                <span className="text-gray-300">| {movie.release_date}</span>
              </p>
              <button
                type="button"
                className="inline-flex items-center rounded-md bg-red-500 px-3 py-2 text-sm font-semibold text-white hover:bg-black/80"
              >
                Watch Now
      ...
      <div className=" absolute inset-0 bg-gradient-to-b from-gray-200/0 via-gray-900/25 to-gray-300 dark:to-[#141414]" />
    </div>
  );
};
